...
const CreateStory = () => {
  ...
  const analyzeStoryPrompt = (prompt, promptSettings) => {
    ...
    const characterMatches = [...prompt.matchAll(/([A-Z]+):/g)].map(
      (match) => match[1]
    );
...
  Ensure proper scene structure, action descriptions, and natural dialogues. Keep character consistency and logical story progression.`;
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are an expert screenplay writer. Write in proper screenplay format.",
              },
  ...
  const steps = [
    {
      title: <>{toggle === false ? "Upload Story" : "Generate Story"}</>,
      content: (
        <div>
          {toggle ? (
            <textarea
          ...
          <div
            ref={contentRef}
            id="content"
            className="mt-6 bg-gray-100 p-4 rounded-lg flex flex-col items-start"
          >
            <h2 className="text-xl font-semibold text-gray-800 mb-2">
              Generated Story
            </h2>
            <p className="text-gray-700 whitespace-pre-line">
              {generatedStory ? generatedStory : defaultContents}
            </p>
          </div>
        </div>
      ),
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg">
      <div className="flex justify-between inline-block align-middle">
        <h1 className="text-2xl font-bold text-gray-800 mb-4">
          Use AI to Generate Your Own Story Script{" "}
          {toggle && <SettingOutlined className="mx-4" onClick={openModal} />}
        </h1>
        <Button type="primary" className="font-medium text-white mx-2 bg-blue hover:bg-blue-dark" onClick={useAiToggle}>
          {" "}
          {toggle ? "Upload Story" : "Use AI"}
        </Button>
      </div>
      ...
      <div className="flex justify-between mt-6">
        ...
        {current === 0 ? (
          <Button onClick={handleGeneratePDF} disabled={!generatedStory}>
